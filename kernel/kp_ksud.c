				  ...
				  kprobe_pre_handler_t handler)
{
	...
	preempt_enable();
	pr_info("kp_ksud: register_%s kprobe: %d\n", name, ret);
	if (ret) {
		kfree(kp);
		return NULL;
	}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0) || defined(CONFIG_KSU_ALLOWLIST_WORKAROUND)
	destroy_kprobe(&key_permission_kp);
#endif
	destroy_kprobe(&input_event_kp);
	destroy_kprobe(&vfs_read_kp);
	destroy_kprobe(&bprm_check_kp);
	// destroy_kprobe(&execve_kp);
	return 0;
}
...
void kp_ksud_init()
{
	// execve_kp = init_kprobe(SYS_EXECVE_SYMBOL, sys_execve_handler_pre);
	bprm_check_kp = init_kprobe("security_bprm_check", bprm_check_handler_pre);
	vfs_read_kp = init_kprobe("vfs_read", vfs_read_handler_pre);
	input_event_kp = init_kprobe("input_event", input_handle_event_handler_pre);

#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0) || defined(CONFIG_KSU_ALLOWLIST_WORKAROUND)
	key_permission_kp = init_kprobe("security_key_permission", key_permission_handler_pre);
#endif
