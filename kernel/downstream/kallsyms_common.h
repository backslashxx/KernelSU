...
static noinline void table_insert(const char *str, uintptr_t addr)
{
	if (!str || !addr)
		return;
...
skip_anti_dup:
	...
	pr_info("%s: hash array resized! %ld -> %ld \n", __func__, old_sz, new_sz);
...
static noinline uintptr_t kallsyms_lookup_name_retry(const char *name)
{
	if (!name)
		return 0x0;
