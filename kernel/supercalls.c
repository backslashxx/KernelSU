...
bool only_root(void)
{
	...
	return FORCE_UID_T(current_uid) == 0;
}

bool manager_or_root(void)
{
	kuid_t current_uid = current_uid();
	return FORCE_UID_T(current_uid) == 0 || is_manager();
}
...
{
	kuid_t current_uid = current_uid();
	bool is_allowed = is_manager() || ksu_is_allow_uid_for_current(FORCE_UID_T(current_uid));
	return is_allowed;
}
