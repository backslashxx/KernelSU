...
// - added 260110
// useless for now, deadcode it
// if YOU want it, just hook sys_newfstat before it returns.
/*
 *	#ifdef CONFIG_KSU
 *	extern int ksu_handle_newfstat_ret(unsigned int *fd, struct stat __user **statbuf_ptr);
 *	#endif
 *	
...
int ksu_handle_newfstat_ret(unsigned int *fd, struct stat __user **statbuf_ptr)
{
	...
	void __user *statbuf = *(void **)statbuf_ptr;
	...
	if (!copy_from_user(&size, st_size_ptr, sizeof(long))) {
		...
		if (!copy_to_user(st_size_ptr, &new_size, sizeof(long)))
			...
			pr_info("add ksu_rc_len failed: statbuf 0x%lx", (unsigned long)st_size_ptr);

	} else
		pr_info("read statbuf 0x%lx failed", (unsigned long)st_size_ptr);
