...
static void ksu_sched_set_fifo()
{
...
has_current_mm:		// HACK: raise priority of this to the heavens
	barrier(); 	// shutup declaration after label
...
int handle_sepolicy(void __user *user_data, u64 data_len)
{
	...
	// ksud is what calls this so process ends anyway
	// we only need to check that its a real process
	// and then raise its priority
	if (current && current->mm)
		ksu_sched_set_fifo();
